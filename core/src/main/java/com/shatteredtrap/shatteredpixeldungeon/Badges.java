...
public class Badges {
	...
	public enum Badge {
		...
		TRAPS_RECLAIMED_1( 72 ),
		TRAPS_RECLAIMED_2( 73 ),
		TRAPS_RECLAIMED_3( 74 ),
		TRAPS_RECLAIMED_4( 75 ),
		TRAPSKILL_ATTAINED_1( 68 ),
		TRAPSKILL_ATTAINED_2( 69 ),
		TRAPSKILL_ATTAINED_3( 70 ),
		TRAPSKILL_ATTAINED_4( 71 ),
		WEIRD_STUFF( 76 );
	...
	public static void validateTrapSkillAttained() {
		Badge badge = null;

		if (!local.contains( Badge.TRAPSKILL_ATTAINED_1 ) && Dungeon.hero.getTrapSkill() >= 1) {
			badge = Badge.TRAPSKILL_ATTAINED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPSKILL_ATTAINED_2 ) && Dungeon.hero.getTrapSkill() >= 2) {
			badge = Badge.TRAPSKILL_ATTAINED_2;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPSKILL_ATTAINED_3 ) && Dungeon.hero.getTrapSkill() >= 3) {
			badge = Badge.TRAPSKILL_ATTAINED_3;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPSKILL_ATTAINED_4 ) && Dungeon.hero.getTrapSkill() >= 4) {
			badge = Badge.TRAPSKILL_ATTAINED_4;
			local.add( badge );
		}
	...
	public static void validateTrapsReclaimed() {
		Badge badge = null;

		if (!local.contains( Badge.TRAPS_RECLAIMED_1 ) && Statistics.trapsReclaimed >= 5) {
			badge = Badge.TRAPS_RECLAIMED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPS_RECLAIMED_2 ) && Statistics.trapsReclaimed >= 10) {
			badge = Badge.TRAPS_RECLAIMED_2;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPS_RECLAIMED_3 ) && Statistics.trapsReclaimed >= 15) {
			badge = Badge.TRAPS_RECLAIMED_3;
			local.add( badge );
		}
		if (!local.contains( Badge.TRAPS_RECLAIMED_4 ) && Statistics.trapsReclaimed >= 20) {
			badge = Badge.TRAPS_RECLAIMED_4;
			local.add( badge );
		}
	...
	public static void validateMageUnlock(){
		if (Statistics.upgradesUsed >= 2000000 && !global.contains(Badge.UNLOCK_MAGE)){
			displayBadge( Badge.UNLOCK_MAGE );
		}
	}
	
	public static void validateRogueUnlock(){
		if (Statistics.sneakAttacks >= 2000000 && !global.contains(Badge.UNLOCK_ROGUE)){
			displayBadge( Badge.UNLOCK_ROGUE );
		}
	}
	
	public static void validateHuntressUnlock(){
		if (Statistics.thrownAssists >= 2000000 && !global.contains(Badge.UNLOCK_HUNTRESS)){
	...
	public static void validateWeirdStuff() {
		if (!local.contains( Badge.WEIRD_STUFF )) {
			Badge badge = Badge.WEIRD_STUFF;
			local.add( badge );
			displayBadge( badge );
		}
	}
	...
	public static List<Badge> filtered( boolean global ) {
		...
		leaveBest( filtered, Badge.TRAPS_RECLAIMED_1, Badge.TRAPS_RECLAIMED_2, Badge.TRAPS_RECLAIMED_3, Badge.TRAPS_RECLAIMED_4 );
		leaveBest( filtered, Badge.TRAPSKILL_ATTAINED_1, Badge.TRAPSKILL_ATTAINED_2, Badge.TRAPSKILL_ATTAINED_3, Badge.TRAPSKILL_ATTAINED_4 );
