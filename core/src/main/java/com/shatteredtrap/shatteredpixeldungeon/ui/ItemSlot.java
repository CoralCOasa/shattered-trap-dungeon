...
import com.shatteredtrap.shatteredpixeldungeon.items.spells.ReclaimTrap2;
...
public class ItemSlot extends Button {
	...
	private void updateText(){
		...
		if (level != 0) {
			...
			if(item instanceof ReclaimTrap && item.quantity()>0){
				if(Dungeon.hero.getTrapSkill()>0){
					if(Dungeon.hero.belongings.getItem( ReclaimTrap.class ).isTrapStored()){
						bottomRight.text( (Dungeon.hero.getCurrentTrapSkill()+1)+"/"+(Dungeon.hero.getTrapSkill()+1));
						bottomRight.measure();
						int color = Dungeon.hero.belongings.getItem(ReclaimTrap.class).storedColor();
						bottomRight.hardlight( color );

						//bottomRight.hardlight( UPGRADED );
					}else{
						bottomRight.text( (0)+"/"+(Dungeon.hero.getTrapSkill()+1));
						bottomRight.measure();
						bottomRight.hardlight( 0xFFFFFF );
				...
				if(Dungeon.hero.getTrapSkill()>0){
					if(Dungeon.hero.belongings.getItem( ReclaimTrap2.class ).isTrapStored()){
						...
						bottomRight.measure();
						int color = Dungeon.hero.belongings.getItem(ReclaimTrap2.class).storedColor();
						bottomRight.hardlight( color );
