...
package com.shatteredtrap.shatteredpixeldungeon.ui.changelist;

import com.shatteredtrap.shatteredpixeldungeon.Assets;
import com.shatteredtrap.shatteredpixeldungeon.Badges;
import com.shatteredtrap.shatteredpixeldungeon.effects.BadgeBanner;
...
import com.shatteredtrap.shatteredpixeldungeon.items.journal.AlchemyPage;
import com.shatteredtrap.shatteredpixeldungeon.items.journal.DocumentPage;
...
import com.shatteredtrap.shatteredpixeldungeon.scenes.ChangesScene;
import com.shatteredtrap.shatteredpixeldungeon.sprites.AssasinSprite;
import com.shatteredtrap.shatteredpixeldungeon.sprites.BombSheepSprite;
...
import com.shatteredtrap.shatteredpixeldungeon.sprites.ChaosElementalSprite;
...
public class v0_7_X_Changes {
	...
	public static void addModChanges( ArrayList<ChangeInfo> changeInfos ){
		ChangeInfo changes = new ChangeInfo("List of changes", true, "");
		...
		changes.addButton( new ChangeButton(Icons.get(Icons.WARNING), "About the mod",
				"Shattered Trap Dungeon is a modded version of Shattered Pixel Dungeon v0.7.4. \n\nThe mod makes traps a more impactful part of the game while also adding other features such as new enemies."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 5*16, 5*16, 16, 16), "Changes to trap system",
						"The amount of traps in the dungeon floors has been greatly increased!\n\n" +
						"There's a bunch of new trap types (see traps section), some of which may appear in trapped vaults.\n\n" +
						"Strong traps will occasionally appear in early floors, too."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 4*16, 6*16, 16, 16), "Changes to Shattered PD traps",
				"Worn dart traps actually do some damage now and the damage scales with depth.\n\n" +
						"Distortion traps are never hidden.\n\n" +
						"Cursing traps only curse one item at a time, and can be used to corrupt enemies!"));

		changes.addButton( new ChangeButton( new ReclaimTrap(),
				"Reclaim traps are dropped by commonly spawning trapper goblins so you can keep using traps against your enemies! \n(Quickslotting is recommended)\n\n" +
						"Reclaim traps can't be dropped to avoid the exploit where reclaimed trap effects could be easily erased.\n\n" +
						"Reclaim traps can be used to preserve a single trap at a time, meaning that you can have two different traps reclaimed at once.\n\n" +
						"Reclaimed traps can be used multiple times if you increase your trapskill by drinking trapskill elixirs."));



		changes.addButton( new ChangeButton(new Image(Assets.TENGU, 0, 0, 14, 16), "Boss reworks",
				"Boss fights are a bit tougher than before.\n\n" +
						"Goo has two enemies from sewers as minions.\n\n" +
						"Tengu doesn't lose a turn when teleporting in the first phase and spawns a guard and a shaman when teleporting in the second phase.\n\n" +
						"DM-300 has three enemies from caves as minions, has armed traps in the arena, and uses goblin protocol when below half hp.\n\n" +
						"King of dwarves is 50% faster and may rarely summon monks and warlocks.\n\n" +
						"Yog has four enemies from halls as minions and spawns some larva automatically."));

		changes.addButton( new ChangeButton( new DemonBeacon(),
				"A single demon beacon generates in each of the three demon halls floors.\n\n" +
						"Demon beacons have no use by itself but finding at least two of them is required in order to activate the Yog fight.\n\n" +
						"This is basically a countermeasure against demon halls rushing.\n\n" +
						"Three demon beacons can be combined by alchemy if you are a fool that's seeking for a challenge."));

		changes.addButton( new ChangeButton(new Image(Assets.RATKING, 0, 0, 16, 16), "WRATH OF THE RAT KING",
				"Rat king summons a new miniboss if woken up."));

		changes.addButton( new ChangeButton( new WeaponRack(),
				"A new container used for storing melee weapons.\n\n" +
						"Dropped by the new rat miniboss."));

		changes.addButton( new ChangeButton( new Image(Assets.WARRIOR, 0, 90, 12, 15), "Warrior subclass buffs",
				"Warrior's subclasses were buffed to make him more useful in this mod.\n\n" +
						"Gladiator has a passive +20% accuracy bonus.\n\n" +
		...
		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.LOOTBAG, new ItemSprite.Glowing(0xFFFFFF)), "Minibosses",
				"There are new randomly appearing minibosses that may be found resting somewhere in the dungeon.\n\n" +
						"They drop a nice treasure bag when defeated, which gives a choice from 5 scrolls or potions."));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.STONE_CLAIRVOYANCE, null), "Runestone room upgrade",
				"Locked runestone rooms now include a collection of traps!\n\n" +
						"Lots of powerful traps can be found inside them for the hero to reclaim."));
		...
		changes.addButton( new ChangeButton(new Image(Assets.GOBLIN, 16, 0, 16, 16), "New enemies",
				"There's a considerable amount of new enemies in the dungeon, see the enemies section for details."));

		changes.addButton( new ChangeButton(new ElixirOfTrapSkill(),
				"Elixir of trapskill is a new powerful elixir that gives a single extra use to all reclaimed traps once consumed!\n\n" +
						"It can be created via alchemy by combining a potion of strength and a scroll of upgrade.\n\n" +
						"Making them later in the game is recommended as getting more strength may no longer be important and there are strong traps to abuse.\n\n" +
						"Its powerful effect combined with its high cost means that the choice of creating them alters the gameplay quite a lot!"));

		changes.addButton( new ChangeButton(new Image(Assets.KEEPER, 0, 0, 14, 16), "Extra shops",
				"In addition to regular shops, there are randomly occurring extra shop rooms in the dungeon!\n\n" +
						"These shops sell scrolls, potions, seeds, runestones, weapons, armor, rings, wands and artifacts.\n\n" +
						"Potions and scrolls sold by the shop may be exotic.\n\n" +
						"Weapons, armor, rings, wands and artifacts are always identified and may be upgraded or cursed."));

		changes.addButton( new ChangeButton( new Gold(1),
				"The amount of gold found in the dungeon has been increased by approximately 33%, enjoy shopping!\n\n" +
						"Exotic items have been made cheaper so that they could be worth buying in extra shops.\n\n" +
						"Bags are cheaper because you'll always buy them so you'll have more money to spend on other cool things.\n\n" +
						"Healing potions are more expensive so that you might not buy them 100% of the time."));

		changes.addButton( new ChangeButton(new Image(Assets.BUFFS_LARGE, 9*16, 32, 16, 16), "Faster game speed",
				"There's an option in settings to increase gameplay speed by speeding up certain animations.\n\n" +
						"It's pretty amazing, try it out!"));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 10*16, 7*16, 16, 16), "Enemy spawning",
				"Enemies can rarely spawn in some weird places such as locked rooms and corridors."));

		changes.addButton( new ChangeButton( new ItemSprite(ItemSpriteSheet.GUIDE_PAGE, null), "Unlockables",
		...
		changes.addButton( new ChangeButton( new AlchemistsToolkit(),
				"A new artifact that replaces the alchemist's toolkit.\n\n" +
						"Its charges can be used to select a position in the hero's field of view, and pull the nearest creature there.\n\n" +
	...
	public static void addEnemyChanges( ArrayList<ChangeInfo> changeInfos ){
		ChangeInfo changes = new ChangeInfo("New enemies", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		changeInfos.add(changes);
		...
		changes.addButton( new ChangeButton(new DancingSkeletonSprite(), "Lesser skeleton",
				"Lesser skeletons are new sewer enemies.\n\n" +
						"They appear very early and have decent stats.\n\n" +
						"However, they skip every second turn which makes it easy to kill them without taking any damage in the process.\n\n" +
						"They may drop stones of intuition when killed."));

		changes.addButton( new ChangeButton(new FilthJetSprite(), "Filth jet",
				"Filth jets are new sewer enemies.\n\n" +
						"They are flying, have a weak magic attack and release toxic gas when killed.\n\n" +
						"Despite their rather annoying properties, they aren't particularly strong.\n\n" +
						"They may drop potions of toxic gas when killed."));

		changes.addButton( new ChangeButton(new OrcSprite(), "Smug orc",
				"Smug orcs are new sewer enemies.\n\n" +
						"Their melee attacks are quite powerful.\n\n" +
						"They don't have other noteworthy properties."));

		changes.addButton( new ChangeButton(new ToughGhostSprite(), "Tough ghost",
				"Tough ghosts are new prison enemies.\n\n" +
						"They are flying and can only be hurt by guaranteed hits.\n\n" +
						"Their attacks are weak but do a bit of extra damage that ignores armor.\n\n" +
						"They may drop scrolls of terror when killed."));
		...
		changes.addButton( new ChangeButton(new MeteorSprite(), "Frost meteor",
				"Frost meteors are new city enemies.\n\n" +
						"They are flying and split up when attacked.\n\n" +
						"They are immune to chilling and freezing and are weak to fire.\n\n"+
						"Each frost meteor creates a chilling 3x3 area when killed."));

		changes.addButton( new ChangeButton(new BombSpider2Sprite(), "Super bomb spider",
				"Super bomb spiders are new city enemies.\n\n" +
						"They have a longer fuse than regular bomb spiders, but explode into a widespread shrapnel blast."));

		changes.addButton( new ChangeButton(new ChaosElementalSprite(), "Chaos elemental",
				"Chaos elementals are new city enemies.\n\n" +
						"They are flying, fast and quite frail.\n\n" +
						"Heroes that get hit by them are randomly teleported.\n\n" +
						"They may drop scrolls of teleportation when killed."));

		changes.addButton( new ChangeButton(new BombSheepSprite(), "Bomb sheep",
				"Bomb sheep appear only when activating annihilation traps.\n\n" +
						"They work like regular sheep but explode instead of vanishing!"));

		changes.addButton( new ChangeButton(new ClownSprite(), "Rat clown",
				"Rat clown is a special miniboss that can be challenged by talking to rat king.\n\n" +
						"Its attacks inflict bleeding and it releases paralyzing stench gas when damaged.\n\n" +
						"At low health it starts running away and summons a hunting animated statue.\n\n" +
						"It always drops a weapon rack when killed."));

		changes.addButton( new ChangeButton(new ItemSprite(ItemSpriteSheet.SOMETHING), "Resting minibosses",
				"The dungeon is home to resting minibosses that you may stumble into.\n\n" +
						"They are quite a bit stronger than regular enemies, and have unique properties.\n\n" +
						"They drop treasure bags when killed."));

		changes.addButton( new ChangeButton(new SpectralSamuraiSprite(), "???",
				"It looks a bit like a rat, doesn't it?\n\n" +
						"Try forbidden alchemy with three demon beacons to find out more..."));
		...
		changes.addButton( new ChangeButton(new AssasinSprite(), "Apprentice assassin",
				"Apprentice assassins are new prison enemies.\n\n" +
						"They can teleport next to players and attack.\n\n" +
						"Their attack damage is quite high.\n\n" +
						"They may drop stones of blink when killed."));

		changes.addButton( new ChangeButton(new KeyChainSprite(), "Haunted keychain",
				"Haunted keychains are new prison enemies.\n\n" +
						"They can hurl keys at nearby traps to activate them.\n\n" +
						"Their attacks deal moderate extra true damage.\n\n" +
						"They may drop exotic scrolls when killed."));

		changes.addButton( new ChangeButton(new BombSpiderSprite(), "Bomb spider",
				"Bomb spiders are new caves enemies.\n\n" +
						"They will explode after getting killed but don't do that immediately so you can back away.\n\n" +
						"Explosions caused by them can destroy items so be careful!"));

		changes.addButton( new ChangeButton(new DartSpitterSprite(), "Spike spitter",
				"Spike spitters are new caves enemies.\n\n" +
						"They have a ranged dart attack.\n\n" +
						"Their movement speed is high and they can get terrified when being attacked at low health.\n\n" +
						"They may drop random missile weapons when killed."));

		changes.addButton( new ChangeButton(new HoleSprite(), "Mobile hole",
				"Mobile holes are new halls enemies.\n\n" +
						"Their attacks will make the target fall down to next floor, similarly to a chasm.\n\n" +
						"Luckily, they move at halved speed and aren't capable of dodging."));

		changes.addButton( new ChangeButton(new DemonRatSprite(), "Demonrat",
				"Demon rats are new halls enemies.\n\n" +
						"Their attacks are powerful and can inflict many different debuffs.\n\n" +
						"They release a cloud of random gas when killed."));

		changes.addButton( new ChangeButton(new PrismancerSprite(), "Prismancer",
				"Prismancers are new halls enemies.\n\n" +
						"They shoot deadly blinding magic beams but take a turn to recharge after shooting.\n\n" +
						"They damage nearby creatures when killed.\n\n" +
						"They may drop a random runestone when killed."));

		changes.addButton( new ChangeButton(new GoblinSprite(), "Trapper goblin",
				"Trapper goblins are new common enemies.\n\n" +
						"They activate traps in a 7x7 area when they are moving while hunting or terrified.\n\n" +
						"There's usually a single trapper goblin per floor.\n\n" +
						"They always drop a reclaim trap spell when killed."));

		//REARRANGED
	}

	public static void addTrapChanges( ArrayList<ChangeInfo> changeInfos ){
		ChangeInfo changes = new ChangeInfo("New traps", true, "");
		changes.hardlight( Window.TITLE_COLOR );
		changeInfos.add(changes);
		...
		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 4*16, 4*16, 16, 16), "Flood trap",
				"Flood trap is a new common hidden trap.\n\n" +
						"It's a quite harmless trap that spreads water to nearby tiles.\n\n" +
						"The water destroys nearby traps which can be helpful."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 3*16, 3*16, 16, 16), "Regrowth trap",
				"Regrowth trap is a new common hidden trap.\n\n" +
						"It spreads grass to nearby tiles and roots creatures.\n\n" +
						"The rooting effect from the trap can last for quite a while."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 5*16, 2*16, 16, 16), "Smokescreen trap",
				"Smokescreen trap is a new common hidden trap.\n\n" +
						"It releases a cloud of shrouding fog that can't been seen through.\n\n" +
						"It's not usually too impactful."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 6*16, 2*16, 16, 16), "Blizzard trap",
				"Blizzard trap is a new uncommon visible trap.\n\n" +
						"It releases a freezing blizzard that spreads like a gas.\n\n" +
						"Everything caught in the blizzard will get chilled and potentially freeze."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 1*16, 5*16, 16, 16), "Cannon trap",
				"Cannon trap is a new uncommon visible trap.\n\n" +
						"It blasts the nearest target with two bombs.\n\n" +
						"Keep in mind that the explosions can destroy items.\n\n" +
						"If the first target is killed by the first bomb, the trap will find a new target for the second one!"));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 0*16, 5*16, 16, 16), "Healing trap",
				"Healing trap is a new rare hidden trap.\n\n" +
						"It fully heals the nearest target.\n\n" +
						"Finding one is always a pleasure, especially because they can be reclaimed."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 2*16, 1*16, 16, 16), "Chestnut trap",
				"Chestnut trap is a new rare hidden trap.\n\n" +
						"It deals 10 damage to the victim.\n\n" +
						"It can be a minor nuisance early on but becomes pointless really quickly."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 0*16, 2*16, 16, 16), "Dream trap",
				"Dream trap is a new rare hidden trap.\n\n" +
						"It releases dreamy gas that makes affected creatures drowsy.\n\n" +
						"It can be used for healing and to put enemies to sleep. However, its healing effect can be hard to utilize at times."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 3*16, 6*16, 16, 16), "Overgrowth trap",
				"Overgrowth trap is a new very rare hidden trap.\n\n" +
						"It fills the entire floor with tall grass.\n\n" +
						"It's a great way to gather dew and seeds though if something catches fire it'll most certainly spread."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 2*16, 6*16, 16, 16), "Disrobing trap",
				"Disrobing trap is a new very rare hidden trap.\n\n" +
						"It removes the victim's armor and teleports the armor somewhere in the floor. The teleported armor will be visible.\n\n" +
						"It has no effect on enemies."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 4*16, 5*16, 16, 16), "Enchanting trap",
				"Enchanting trap is a new very rare hidden trap.\n\n" +
						"It enchants equipped weapon and/or armor.\n\n" +
						"You can throw an item on the trap to enchant it as well."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 2*16, 4*16, 16, 16), "Lottery trap",
				"Gambling trap is a new very rare visible trap.\n\n" +
						"It reduces or increases your strength by one or alternatively doubles or removes your money.\n\n" +
						"It can help or cause trouble but because it's visible you may just choose to ignore it."));
		...
		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 1*16, 2*16, 16, 16), "Inferno trap",
				"Inferno trap is a new uncommon visible trap.\n\n" +
						"It releases an inferno that spreads like a gas.\n\n" +
						"Everything caught in the inferno will be set on fire."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 5*16, 3*16, 16, 16), "Spook trap",
				"Spook trap is a new uncommon hidden trap.\n\n" +
						"It spawns wraiths around it similarly to an activated tombstone."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 6*16, 5*16, 16, 16), "Cryo trap",
				"Cryo trap is a new uncommon visible trap.\n\n" +
						"It affects the nearest target with an insanely strong freeze.\n\n" +
						"Creatures frozen by this trap will never thaw out unless they take damage."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 5*16, 4*16, 16, 16), "Spawning trap",
				"Spawning trap is a new uncommon visible trap.\n\n" +
						"It spawns lots of monsters around the floor."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 1*16, 6*16, 16, 16), "Sharpnel trap",
				"Sharpnel trap is a new rare hidden trap.\n\n" +
						"It damages creatures in a huge area.\n\n" +
						"Its effect is identical to a sharpnel bomb explosion."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 3*16, 4*16, 16, 16), "Goblin trap",
				"Goblin trap is a new rare hidden trap.\n\n" +
						"It summons terrified trapper goblins around it.\n\n" +
						"The goblins with quickly start wandering and will most likely activate lots of nearby traps."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 1*16, 1*16, 16, 16), "Annihilation trap",
				"Annihilation trap is a new rare visible trap.\n\n" +
						"It summons a flock of explosive sheep.\n\n" +
						"The sheep stay still and will quickly start exploding."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 2*16, 5*16, 16, 16), "Mimic trap",
				"Mimic trap is a new rare visible trap.\n\n" +
						"It summons a powerful mimic that's 8 levels stronger than a generic mimic.\n\n" +
						"The mimic drops a wand, weapon, armor or ring that has a single extra upgrade in it.\n\n" +
						"The loot dropped by the mimic can be quite useful though getting killed by the mimic can be a serious concern, especially early on."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 0*16, 1*16, 16, 16), "Fury trap",
				"Fury trap is a new uncommon hidden trap.\n\n" +
						"It affects the target with magical fury, which makes it deal and take double damage.\n\n" +
						"The buff can be dangerous but holds immense killing potential, whether it's applied to the hero or an enemy."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 2*16, 2*16, 16, 16), "Rearming trap",
				"Rearming trap is a new rare hidden trap.\n\n" +
						"It rearms all activated traps in the floor (except itself).\n\n" +
						"It could make an annoying trap block a path again, or be useful for reclaiming purposes.\n\n" +
						"If you were to find two rearming traps on the same floor, however..."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 6*16, 6*16, 16, 16), "Marking trap",
				"Marking trap is a new rare hidden trap.\n\n" +
						"It shoots a beam at the nearest target, marking it.\n\n" +
						"If a marked creature gets hit by a beam from the trap again, the mark is consumed to deal very high damage.\n\n" +
						"Marking trap doesn't break when triggered."));

		changes.addButton( new ChangeButton( new Image(Assets.TERRAIN_FEATURES, 7*16, 1*16, 16, 16), "Slicing trap",
