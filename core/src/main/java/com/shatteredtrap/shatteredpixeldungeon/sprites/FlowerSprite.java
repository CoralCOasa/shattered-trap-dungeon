...
public class FlowerSprite extends MobSprite {

	public FlowerSprite() {
		super();
		
		texture( Assets.FLOWER );
		
		TextureFilm frames = new TextureFilm( texture, 20, 20 );
		
		idle = new Animation( 2, true );
		idle.frames( frames, 0, 0, 1, 0, 0, 0, 1 );
		
		run = new Animation( 15, true );
		run.frames( frames, 0, 1 );
		
		attack = new Animation( 5/ SPDSettings.speeder2(), false );
		attack.frames( frames, 3, 2 );
