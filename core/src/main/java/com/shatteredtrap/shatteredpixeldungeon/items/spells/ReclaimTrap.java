...
public class ReclaimTrap extends TargetedSpell {
	...
	public static final String AC_PRESERVE = "PRESERVE";
	...
	public void execute( final Hero hero, String action ) {
		...
		if (action.equals( AC_CAST )) {
		...
		}else if (action.equals( AC_PRESERVE )){
			if(storedTrap==null) {
			...
			if (preserved != null){
				...
				hero.setCurrentTrapSkill2(hero.getCurrentTrapSkill());
				hero.setCurrentTrapSkill(0);
				ReclaimTrap2 t2 = new ReclaimTrap2();
				t2.storeTrap(this.storedTrap);
				if (t2.doPickUp( Dungeon.hero )) {
				} else {
					Dungeon.level.drop( t2, Dungeon.hero.pos ).sprite.drop();
				}
				storedTrap = null;
				quantity--;
				updateQuickslot();
		...
		if (storedTrap == null) {
			...
			try {
				...
				if (hero.getCurrentTrapSkill() != 0 ){
					...
					hero.setCurrentTrapSkill(hero.getCurrentTrapSkill()-1);
